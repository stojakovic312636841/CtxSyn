...
def build_gauss_kernel(size=5, sigma=1.0, n_channels=1, cuda=0):
    if size % 2 != 1:
...
class LapLoss(nn.Module):
    def __init__(self, max_levels=5, k_size=5, sigma=2.0,gpu_id = 0):
    ...
    def forward(self, input, target):
        if self._gauss_kernel is None or self._gauss_kernel.shape[1] != input.shape[1]:
            self._gauss_kernel = build_gauss_kernel(
                ...
                n_channels=input.shape[1], cuda=self.gpu_id
            )
