...
from modules import LateralBlock, DownSamplingBlock, UpSamplingBlock,FirstConvBlock
...
class GridNet(nn.Module):
    def __init__(self, in_chs, out_chs, grid_chs = [32, 64, 96]):
        ...
        self.lateral_init = FirstConvBlock(in_chs, self.n_chs[0])
        
        for r, n_ch in enumerate(self.n_chs):
            for c in range(self.n_col-1):
                setattr(self, 'lateral_%d_%d'%(r,c), LateralBlock(n_ch, n_ch))

        for r, (in_ch, out_ch) in enumerate(zip(self.n_chs[:-1], self.n_chs[1:])):
            for c in range(int(self.n_col/2)):
                #print('down_%d_%d'%(r,c))
                setattr(self, 'down_%d_%d'%(r,c), DownSamplingBlock(in_ch, out_ch))

        for r, (in_ch, out_ch) in enumerate(zip(self.n_chs[1:], self.n_chs[:-1])):
            for c in range(int(self.n_col/2)):
                setattr(self, 'up_%d_%d'%(r,c), UpSamplingBlock(in_ch, out_ch))
